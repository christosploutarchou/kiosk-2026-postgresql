        ...
        ElseIf printType = "X" Then
            ...
            Try
                sql = "select from_date, to_date, total_receipts, total5percent, total19percent, payments, " &
                      "initial_amt, final_amt, description, total0percent, amount_laxeia, initialAmtLaxeia, amountvisa, total3percent  " &
                      "from x_report " &
        ...
        If btnHold.Text = HOLD Then
            ...
            Try
                For i As Integer = 0 To dgvReceipt.Rows.Count - 1
                    tmpTrxn.dgvReceipt.Rows.Add(dgvReceipt.Rows(i).Cells("productSerno").Value(),
                                                dgvReceipt.Rows(i).Cells("serno").Value(),
                                                dgvReceipt.Rows(i).Cells("description").Value(),
                                                dgvReceipt.Rows(i).Cells("quantity").Value(),
                                                dgvReceipt.Rows(i).Cells("unitprice").Value(),
                                                dgvReceipt.Rows(i).Cells("amount").Value(),
                                                dgvReceipt.Rows(i).Cells("vat").Value(),
                                                dgvReceipt.Rows(i).Cells("isKronos").Value(),
                                                dgvReceipt.Rows(i).Cells("itemCode").Value(),
                                                dgvReceipt.Rows(i).Cells("issueNumber").Value(),
                                                dgvReceipt.Rows(i).Cells("deliveryDate").Value()
                                                )
                Next
            Catch ex As Exception
                CreateExceptionFile(ex.Message, " ")
